...
def train(
    ...
    for step in range(1,  args.first_epoch + 1):
        ...
        final_index = (torch.ones(batch_size, device=device) * (args.seq_len - 1))
        time = time[:x.size(1)]
    ...
    embedder.load_state_dict(torch.load(path/"embedder.pt", map_location=device))
    recovery.load_state_dict(torch.load(path/"recovery.pt", map_location=device))
    ...
    for step in range(1, max_steps+1):
        ...
        if step % 500 == 0:
            ...
            with torch.no_grad():
                ...
                final_index = (torch.ones(dataset_size, device=device) * (args.seq_len - 1))
